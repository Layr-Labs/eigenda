...
// helper method to create a CachedOnchainState for testing, fetching the operator state for the given reference block
// number and setting the current time as LastRefreshed.
func createCachedState(t *testing.T, ctx context.Context, referenceBlockNumber uint64) *controller.CachedOnchainState {
	state, err := mockChainState.GetIndexedOperatorState(ctx, uint(referenceBlockNumber), []core.QuorumID{0, 1})
...
func TestDispatcherHandleBatch(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Standard mock expectations for blobsets
			...
			operatorState, err := mockChainState.GetIndexedOperatorState(ctx, uint(referenceBlockNumber), []core.QuorumID{0, 1})
			require.NoError(t, err)
			...
			if tc.useCache {
				// Set cache
				cachedState := createCachedState(t, ctx, referenceBlockNumber)
			...
			// Test that the blob metadata status is updated
			...
			// close heartbeat channel and assert
...
func TestDispatcherInsufficientSignatures(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			certs := append(objsInBothQuorum.blobCerts, objsInQuorum1.blobCerts...)
			...
			if tc.setupCache {
				...
				// Legacy: Force fresh fetch for every batch, so the cache is not set
				mockChainState.On("GetIndexedOperatorState", mock.Anything, mock.Anything, mock.Anything).
					Return(createCachedState(t, ctx, referenceBlockNumber).OperatorState, nil)
			}
...
func TestDispatcherMaxBatchSize(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Always return a valid operator state for any call
			cachedState := createCachedState(t, ctx, 90)
			mockChainState.On("GetIndexedOperatorState", mock.Anything, mock.Anything, mock.Anything).
				Return(cachedState.OperatorState, nil)
			components.Dispatcher.SetCachedOnchainState(cachedState)
			require.NotNil(t, components.Dispatcher.GetCachedOnchainState())
...
func TestDispatcherNewBatch(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Always return a valid operator state for any call
			cachedState := createCachedState(t, ctx, tc.blockNumber)
			mockChainState.On("GetIndexedOperatorState", mock.Anything, mock.Anything, mock.Anything).
				Return(cachedState.OperatorState, nil)
			components.Dispatcher.SetCachedOnchainState(cachedState)
			require.NotNil(t, components.Dispatcher.GetCachedOnchainState())
			...
			require.Equal(t, tc.blockNumber, batch.BatchHeader.ReferenceBlockNumber)
...
func TestDispatcherNewBatchFailure(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Len(t, newBatchData.Batch.BlobCertificates, 1)
