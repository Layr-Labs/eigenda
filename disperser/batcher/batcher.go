...
func (b *Batcher) updateConfirmationInfo(ctx context.Context, batch *batch, aggSig *core.SignatureAggregation, txnReceipt *types.Receipt) ([]*disperser.BlobMetadata, error) {
	...
	for blobIndex, metadata := range batch.BlobMetadata {
		...
		var proof []byte
		if isBlobAttested(aggSig.QuorumResults, batch.BlobHeaders[blobIndex]) {
...
func (b *Batcher) ProcessConfirmedBatch(ctx context.Context, receiptOrErr *ReceiptOrErr) error {
	...
	batch := confirmationMetadata.batch
	if batch == nil {
		return fmt.Errorf("failed to process confirmed batch: batch from transaction manager metadata is nil")
	}
	...
	if confirmationMetadata.aggSig == nil {
		_ = b.handleFailure(ctx, batch.BlobMetadata, FailNoAggregatedSignature)
	...
	stageTimer := time.Now()
	blobsToRetry, err := b.updateConfirmationInfo(ctx, batch, confirmationMetadata.aggSig, receiptOrErr.Receipt)
	if err != nil {
		_ = b.handleFailure(ctx, batch.BlobMetadata, FailUpdateConfirmationInfo)
		return fmt.Errorf("failed to update confirmation info: %w", err)
	}
	if len(blobsToRetry) > 0 {
		b.logger.Error("failed to update confirmation info", "failed", len(blobsToRetry), "total", len(batch.BlobMetadata))
