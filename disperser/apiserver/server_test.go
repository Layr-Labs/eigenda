...
func TestQuorumRolloutReadinessCalculation(t *testing.T) {
	// Setup fake operator states
	q0 := core.QuorumID(0)
	q1 := core.QuorumID(1)
	requiredVersion := "1.2.3"
	threshold := 0.8
	// 3 operators: 2 in q0, 2 in q1, 1 in both
	ops := map[core.OperatorID]*apiserver.OperatorOnchainState{}
	opA := core.OperatorID{0x01}
	opB := core.OperatorID{0x02}
	opC := core.OperatorID{0x03}
	ops[opA] = &apiserver.OperatorOnchainState{
		OperatorID: opA,
		Stake:      big.NewInt(100),
		NodeInfo:   &pbvalidator.GetNodeInfoReply{Semver: requiredVersion},
		Quorums:    []core.QuorumID{q0},
	}
	ops[opB] = &apiserver.OperatorOnchainState{
		OperatorID: opB,
		Stake:      big.NewInt(100),
		NodeInfo:   &pbvalidator.GetNodeInfoReply{Semver: "old"},
		Quorums:    []core.QuorumID{q0, q1},
	}
	ops[opC] = &apiserver.OperatorOnchainState{
		OperatorID: opC,
		Stake:      big.NewInt(200),
		NodeInfo:   &pbvalidator.GetNodeInfoReply{Semver: requiredVersion},
		Quorums:    []core.QuorumID{q1},
	}
	// Call the readiness calculation logic (factored out for testability)
	pctByQuorum, readyByQuorum := apiserver.CalculateQuorumRolloutReadiness(ops, requiredVersion, threshold)
	// q0: opA (100, new), opB (100, old) => 100/200 = 0.5 < 0.8
	// q1: opB (100, old), opC (200, new) => 200/300 = 0.666... < 0.8
	assert.Equal(t, float64(0.5), pctByQuorum[q0])
	assert.False(t, readyByQuorum[q0])
	// Now upgrade opB
	ops[opB].NodeInfo.Semver = requiredVersion
	pctByQuorum, readyByQuorum = apiserver.CalculateQuorumRolloutReadiness(ops, requiredVersion, threshold)
	// q0: opA (100), opB (100) => 200/200 = 1.0
	// q1: opB (100), opC (200) => 300/300 = 1.0
	assert.Equal(t, float64(1.0), pctByQuorum[q0])
	assert.True(t, readyByQuorum[q0])

	// Edge case 1: No operators
	pctByQuorum, readyByQuorum = apiserver.CalculateQuorumRolloutReadiness(map[core.OperatorID]*apiserver.OperatorOnchainState{}, requiredVersion, threshold)
	...
	ops2 := map[core.OperatorID]*apiserver.OperatorOnchainState{
		opD: {
			OperatorID: opD,
			Stake:      big.NewInt(0),
			NodeInfo:   &pbvalidator.GetNodeInfoReply{Semver: requiredVersion},
	...
	pctByQuorum, readyByQuorum = apiserver.CalculateQuorumRolloutReadiness(ops2, requiredVersion, threshold)
	assert.Equal(t, float64(0), pctByQuorum[q0])
	assert.False(t, readyByQuorum[q0])

	// Edge case 3: Operator with nil NodeInfo
	opE := core.OperatorID{0x05}
	ops3 := map[core.OperatorID]*apiserver.OperatorOnchainState{
		opE: {
			OperatorID: opE,
			Stake:      big.NewInt(100),
			NodeInfo:   nil,
	...
	pctByQuorum, readyByQuorum = apiserver.CalculateQuorumRolloutReadiness(ops3, requiredVersion, threshold)
	assert.Equal(t, float64(0), pctByQuorum[q0])
	assert.False(t, readyByQuorum[q0])
	...
	ops4 := map[core.OperatorID]*apiserver.OperatorOnchainState{
		opF: {
			OperatorID: opF,
			Stake:      big.NewInt(80),
			NodeInfo:   &pbvalidator.GetNodeInfoReply{Semver: requiredVersion},
	...
	pctByQuorum, readyByQuorum = apiserver.CalculateQuorumRolloutReadiness(ops4, requiredVersion, 0.8)
	assert.Equal(t, float64(0.8), pctByQuorum[q0])
	assert.True(t, readyByQuorum[q0])

	// Edge case 5: All upgraded except one with tiny stake
	opH := core.OperatorID{0x08}
	ops5 := map[core.OperatorID]*apiserver.OperatorOnchainState{
		opA: ops[opA],
		opB: ops[opB],
		opC: ops[opC],
		opH: {
	...
	// All q0 except opH are upgraded, but threshold is not met
	pctByQuorum, readyByQuorum = apiserver.CalculateQuorumRolloutReadiness(ops5, requiredVersion, 0.999)
	assert.True(t, pctByQuorum[q0] < 0.999)
	assert.False(t, readyByQuorum[q0])

	// Edge case 6: Operator in multiple quorums, only upgraded in some
	opI := core.OperatorID{0x09}
	ops6 := map[core.OperatorID]*apiserver.OperatorOnchainState{
		opI: {
			OperatorID: opI,
			Stake:      big.NewInt(100),
			NodeInfo:   &pbvalidator.GetNodeInfoReply{Semver: requiredVersion},
	...
	pctByQuorum, readyByQuorum = apiserver.CalculateQuorumRolloutReadiness(ops6, requiredVersion, threshold)
	assert.Equal(t, float64(0.5), pctByQuorum[q0])
	assert.Equal(t, float64(2.0/3.0), pctByQuorum[q1])
	assert.False(t, readyByQuorum[q0])
