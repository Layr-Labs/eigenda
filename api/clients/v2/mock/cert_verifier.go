...
// MockCertVerifier is an autogenerated mock type for the ICertVerifier type
...
// GetQuorumNumbersRequired provides a mock function with given fields: ctx, certVerifierAddress
func (_m *MockCertVerifier) GetQuorumNumbersRequired(ctx context.Context, certVerifierAddress string) ([]uint8, error) {
	ret := _m.Called(ctx, certVerifierAddress)

	if len(ret) == 0 {
		panic("no return value specified for GetQuorumNumbersRequired")
	}

	var r0 []uint8
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, string) ([]uint8, error)); ok {
		return rf(ctx, certVerifierAddress)
	}
	if rf, ok := ret.Get(0).(func(context.Context, string) []uint8); ok {
		r0 = rf(ctx, certVerifierAddress)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]uint8)
		}
