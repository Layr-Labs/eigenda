...
func (a *Accountant) validateQuorumReservations(quorumNumbers []uint8, timestamp time.Time) error {
	...
	for _, quorumNumber := range quorumNumbers {
		...
		if reservation, exists := a.reservations[core.QuorumID(quorumNumber)]; !exists {
			...
			if !core.CheckTimeRange(time.Unix(int64(reservation.StartTimestamp), 0), time.Unix(int64(reservation.EndTimestamp), 0), timestamp) {
...
func (a *Accountant) BlobPaymentInfo(
	...
	numSymbols uint64,
	quorumNumbers []uint8,
	timestamp time.Time) (*big.Int, error) {
...
func (a *Accountant) ReservationUsage(numSymbols uint64, quorumNumbers []uint8, timestamp time.Time) (*big.Int, error) {
	...
	for _, quorumNumber := range quorumNumbers {
		...
		currentReservationPeriod := meterer.GetReservationPeriod(timestamp.Unix(), reservationWindow)
