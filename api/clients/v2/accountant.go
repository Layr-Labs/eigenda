...
func (a *Accountant) BlobPaymentInfo(ctx context.Context, numSymbols uint32, quorumNumbers []uint8) (uint32, *big.Int, error) {
	...
	fmt.Println("accoutant calcu", "relativeBinRecord", relativeBinRecord.Usage)
	...
	relativeBinRecord.Usage -= uint64(numSymbols)
	incrementRequired := big.NewInt(int64(a.PaymentCharged(numSymbols)))
	fmt.Println("check ondemand", incrementRequired, "local cumulativePayment", a.cumulativePayment, "ondemand onchain", a.onDemand.CumulativePayment, "quorum numbers", "quorumNumbers", "requiredQuorums", requiredQuorums)
