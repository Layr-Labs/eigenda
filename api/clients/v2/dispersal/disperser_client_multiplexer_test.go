...
func createTestMultiplexer(
	...
	dcm := NewDisperserClientMultiplexer(
		logger,
		config,
		mockRegistry,
		signer,
...
func TestGetDisperserClient_WithOnDemandPaymentFilter(t *testing.T) {
	...
	// Disperser 2 has best reputation but is NOT on-demand
	// Should choose disperser 3 (second-worst, but IS on-demand)
	client, err := multiplexer.GetDisperserClient(t.Context(), time.Now(), true)
	require.NoError(t, err)

	disperserID := client.GetConfig().DisperserID
	require.Equal(t, uint32(3), disperserID, "should choose best on-demand disperser")
}

func TestGetDisperserClient_ChoosesBestReputation(t *testing.T) {
	config := DefaultDisperserClientMultiplexerConfig()
	multiplexer, _ := createTestMultiplexer(t, config)

	// Disperser 2 has the best reputation
	client, err := multiplexer.GetDisperserClient(t.Context(), time.Now(), false)
	require.NoError(t, err)
	require.Equal(t, uint32(2), client.GetConfig().DisperserID, "should choose best reputation")

	cachedClient, err := multiplexer.GetDisperserClient(t.Context(), time.Now(), false)
	require.NoError(t, err)
	require.Same(t, client, cachedClient, "should return cached client")
}
...
func TestGetDisperserClient_CleansUpOutdatedClient(t *testing.T) {
	...
	// Disperser 2 has best reputation from setup, so it will be chosen
...
func TestGetDisperserClient_AdditionalDispersersAndBlacklist(t *testing.T) {
	...
	// Disperser 2 (best reputation) is blacklisted
	// Dispersers 1 and 3 have degraded reputations
	// Disperser 4 has neutral default reputation (no reports yet)
	client, err := multiplexer.GetDisperserClient(t.Context(), time.Now(), false)
	require.NoError(t, err)
	require.Equal(t, uint32(4), client.GetConfig().DisperserID)
}
