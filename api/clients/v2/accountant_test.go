...
func TestAccountBlob_Reservation(t *testing.T) {
	...
	currentPeriod := meterer.GetReservationPeriod(now.Unix(), window) % uint64(numBins)
	...
	overflowIndex := uint32((meterer.GetReservationPeriod(now.Unix(), window) + 2) % uint64(numBins))
...
func TestAccountBlobCallSeries(t *testing.T) {
	...
	assert.Equal(t, uint64(meterer.GetReservationPeriod(now.Unix(), reservationWindow)), uint64(meterer.GetReservationPeriod(int64(timestamp), reservationWindow)))
	assert.Equal(t, big.NewInt(0), header.CumulativePayment)
...
func TestAccountBlob_BinRotation(t *testing.T) {
	...
	// First call
	now := time.Now()
	currentPeriod := meterer.GetReservationPeriod(now.Unix(), reservationWindow)
	...
	// Second call
	now = now.Add(time.Duration(reservationWindow) * time.Second)
	nextPeriod := meterer.GetReservationPeriod(now.Unix(), reservationWindow)
...
func TestBlobPaymentInfo_UseReservation(t *testing.T) {
	...
	reservations := map[uint8]*core.QuorumReservation{
		0: {
			SymbolsPerSecond: 100,
			StartTimestamp:   uint32(now.Add(-1 * time.Hour).Unix()),
			EndTimestamp:     uint32(now.Add(1 * time.Hour).Unix()),
		},
		1: {
			SymbolsPerSecond: 200,
			StartTimestamp:   uint32(now.Add(-1 * time.Hour).Unix()),
...
func TestBlobPaymentInfo_FutureReservation(t *testing.T) {
	...
	reservations := map[uint8]*core.QuorumReservation{
		0: {
			SymbolsPerSecond: 100,
			StartTimestamp:   uint32(now.Add(1 * time.Hour).Unix()), // Future reservation
...
func TestBlobPaymentInfo_MultipleOverflows(t *testing.T) {
	...
	require.NoError(t, err)
	currentPeriod := meterer.GetReservationPeriod(now.Unix(), 1)
...
func TestBlobPaymentInfo_MixedReservationStates(t *testing.T) {
	...
	payment, err := acc.BlobPaymentInfo(ctx, 50, []uint8{0, 1, 2}, now)
	assert.Nil(t, payment)
	assert.Contains(t, err.Error(), "not allowed")

	// Separate reservation dispersal is sufficient for quorum 2
	payment, err = acc.BlobPaymentInfo(ctx, 50, []uint8{2}, now)
	require.NoError(t, err)
	assert.NotNil(t, payment)
	assert.True(t, payment.Cmp(big.NewInt(0)) == 0)

	// Alternatively use ondemand for quorum 0 or/and 1
	payment, err = acc.BlobPaymentInfo(ctx, 50, []uint8{0, 1}, now)
	require.NoError(t, err)
	assert.NotNil(t, payment)
	assert.True(t, payment.Cmp(big.NewInt(0)) > 0)
