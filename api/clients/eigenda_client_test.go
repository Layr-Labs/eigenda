...
func TestPutRetrieveBlobIFFTSuccess(t *testing.T) {
	...
	blobInfo, err := eigendaClient.PutBlob(ctx, expectedBlob)
	require.NoError(t, err)
	require.NotNil(t, blobInfo)
	assert.Equal(t, finalizedBlobInfo, blobInfo)

	resultBlob, err := eigendaClient.GetBlob(ctx, []byte("mock-batch-header-hash"), 100)
	require.NoError(t, err)
...
func TestPutBlobFailDispersal(t *testing.T) {
	...
	blobInfo, err := eigendaClient.PutBlob(ctx, []byte("hello"))
	require.Error(t, err)
