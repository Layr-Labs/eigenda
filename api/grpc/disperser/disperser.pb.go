...
type DisperseBlobReply struct {
	...
	//
	// Once a request is accepted, and before it is processed, a unique request ID is generated.
	...
	//
	// Two different DisperseBlobRequests thus have different IDs, whereas the same DisperseBlobRequest
	// sent repeatedly at different times will have different IDs. Clients should thus store this ID
	// and use it to query the processing status of the request via the GetBlobStatus API.
