...
type PaymentHeader struct {
	...
	// as well as the recorded payments for all previous requests.
	//
	...
	//  2. The first time a user makes a request, server will immediate read the contract for the on-chain balance. When user runs
	//     out of on-chain balance, the server will reject the dispersal request. When a user tops up on-chain,
	//     the server will refresh every few minutes for the user top-up to take effect.
	//  3. The disperser client accounts how many tokens they’ve already paid (previousCumPmt).
	//  4. Disperser client calculates the price of a request by rounding up blob size to the nearest multiple of `minNumSymbols`
	//     and then multiply by `pricePerSymbol` as defined by the payment vault contract. Client considers it as the required charge
	//     of the blob dispersal request.
	//  4. Dispersal client takes the sum of previousCumPmt + charge and place it in the “cumulative_payment” field as part of the request header.
	//  5. The disperser checks this new cumulative total against on-chain deposits and the largest seen payment from the same client.
	//  6. If the “cumulative_payment” field is not valid, the request is rejected.
	//  7. If the “cumulative_payment” field is valid, the disperser will check the request usage against the current global ratelimiting period's
	//     on-demand usage. If the new global on-demand usage doesn't exceed the maximum, then dispersal request should proceed. If the disperser
	//     the new usage does exceed the maximum, then dispersal request is rejected with payments not charged against the client.
