...
func DeprecatedCLIFlags(envPrefix, category string) []cli.Flag {
	return []cli.Flag{
		...
		&cli.DurationFlag{
			...
			Action: func(*cli.Context, time.Duration) error {
				return fmt.Errorf("flag --%s (env var %s) is deprecated, use --%s (env var %s) instead",
					DeprecatedStatusQueryRetryIntervalFlagName,
					withDeprecatedEnvPrefix(envPrefix, "STATUS_QUERY_INTERVAL"),
					StatusQueryRetryIntervalFlagName,
					withEnvPrefix(envPrefix, "STATUS_QUERY_INTERVAL"))
			},
			Hidden: true,
		},
		&cli.BoolFlag{
		...
		&cli.StringFlag{
			...
			Usage: `Hex-encoded signer private key. This key should not be associated with an Ethereum address
						holding any funds.`,
		...
		&cli.UintFlag{
			...
			Action: func(*cli.Context, uint) error {
				return fmt.Errorf("flag --%s (env var %s) is deprecated, use --%s (env var %s) instead",
					DeprecatedPutBlobEncodingVersionFlagName,
					withDeprecatedEnvPrefix(envPrefix, "PUT_BLOB_ENCODING_VERSION"),
					PutBlobEncodingVersionFlagName,
					withEnvPrefix(envPrefix, "PUT_BLOB_ENCODING_VERSION"))
			},
			Hidden: true,
		},
		&cli.BoolFlag{
			...
			Action: func(*cli.Context, bool) error {
				return fmt.Errorf("flag --%s (env var %s) is deprecated, use --%s (env var %s) instead",
					DeprecatedDisablePointVerificationModeFlagName,
					withDeprecatedEnvPrefix(envPrefix, "DISABLE_POINT_VERIFICATION_MODE"),
					DisablePointVerificationModeFlagName,
					withEnvPrefix(envPrefix, "DISABLE_POINT_VERIFICATION_MODE"))
			},
			Hidden: true,
		},
	}
