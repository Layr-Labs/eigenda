...
func generateRequest(size int64) (*pb.StoreChunksRequest, error) {
	log.Println("Building request of size", size)
	// Create a BatchHeader
	batchHeader := &pb.BatchHeader{
		BatchRoot:            []byte("root_hash_example"),
	...
	blob := &pb.Blob{
		Header:  blobHeader,
		Bundles: []*pb.Bundle{bundle},
	}

	// Create the StoreChunksRequest
	storeChunksRequest := &pb.StoreChunksRequest{
		BatchHeader: batchHeader,
		Blobs:       []*pb.Blob{blob},
	}

	return storeChunksRequest, nil
}
