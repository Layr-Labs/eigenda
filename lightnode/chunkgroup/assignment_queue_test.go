...
func randomAssignment(nextShuffleTime time.Time) *chunkGroupAssignment {
	...
	assignmentIndex := uint64(rand.Intn(64))
...
func TestInOrderInsertion(t *testing.T) {
	...
	for i := uint(0); i < numberOfElements; i++ {
		...
		assert.Equal(t, uint64(i+1), queue.Size())
	}
	...
	for i := uint(0); i < numberOfElements; i++ {
		...
		assert.Equal(t, uint64(numberOfElements-i), queue.Size())

		next := queue.Pop()
		assert.Equal(t, expectedOrder[i], next)
		assert.Equal(t, uint64(numberOfElements-i-1), queue.Size())
	}
}
...
func TestReverseOrderInsertion(t *testing.T) {
	...
	for i := int(numberOfElements - 1); i >= 0; i-- {
		...
		assert.Equal(t, numberOfElements-uint64(i), queue.Size())
	}
...
func TestPeriodicRemoval(t *testing.T) {
	...
	for i := uint(0); i < numberOfElements; i++ {
		if i%7 == 0 {
			...
			removalCount++
			assert.Equal(t, uint64(numberOfElements-removalCount), queue.Size())
		}
	}
	...
	for i := uint(0); i < (numberOfElements - removalCount); i++ {
		...
		assert.Equal(t, uint64(numberOfElements-i-removalCount), queue.Size())

		next := queue.Pop()
		assert.Equal(t, expectedOrderWithRemovals[i], next)
		assert.Equal(t, uint64(numberOfElements-i-removalCount-1), queue.Size())
	}
}
