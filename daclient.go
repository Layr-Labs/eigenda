...
// ForkedDAClient
type ForkedDAClient struct {
	url    string
	verify bool
}

func NewForkedDAClient(url string, verify bool) *ForkedDAClient {
	return &ForkedDAClient{url, verify}
}
...
// SetInput sets the input data and returns the EigenDA blob commitment.
func (c *ForkedDAClient) SetInput(ctx context.Context, img []byte) (EigenDACommitment, error) {
	if len(img) == 0 {
		return nil, ErrInvalidInput
	}
...
func (c *ForkedDAClient) Health() bool {
