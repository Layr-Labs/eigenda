...
func KzgConfigFromV1Config(v1 *kzgconfig.Config) *KzgConfig {
	return &KzgConfig{
...
func ReadCLIConfig(ctx *cli.Context) KzgConfig {
	cfg := KzgConfig{
		SRSNumberToLoad: ctx.GlobalUint64(kzgconfig.SRSLoadingNumberFlagName),
		G1Path:          ctx.GlobalString(kzgconfig.G1PathFlagName),
		CacheDir:        ctx.GlobalString(kzgconfig.CachePathFlagName),
		NumWorker:       ctx.GlobalUint64(kzgconfig.NumWorkerFlagName),
		PreloadEncoder:  ctx.GlobalBool(kzgconfig.PreloadEncoderFlagName),
	}
	return cfg
}
