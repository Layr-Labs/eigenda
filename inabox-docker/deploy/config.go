...
func (env *Config) generateChurnerVars(ind int, graphUrl, logPath, grpcPort string) ChurnerVars {
	...
	fmt.Println(v.CHURNER_HOSTNAME)
...
func (env *Config) generateOperatorVars(ind int, name, key, churnerUrl, logPath, dbPath, dispersalPort, retrievalPort, metricsPort, nodeApiPort string) OperatorVars {
	...
	blsKey := env.Pks.BlsMap[name].PrivateKey
	ecdsaKey := env.Pks.EcdsaMap[name].PrivateKey
...
func (env *Config) generateRetrieverVars(ind int, graphUrl, logPath, grpcPort string) RetrieverVars {
	v := RetrieverVars{
		RETRIEVER_HOSTNAME:                    "",
...
func (env *Config) genService(compose testbed, name, image, dockerfilePath, envFile string, ports []string) {
	...
	compose.Services[name] = map[string]interface{}{
		// "image":    image,
		"env_file": []string{filepath.Join(env.Path, envFile)},
		"ports":    ports,
		"volumes": []string{
			env.Path + ":/data",
			env.rootPath + "/inabox-docker/secrets:/secrets",
			env.rootPath + "/inabox-docker/resources:/resources",
		},
		"build": Build{
		...
		"depends_on": map[string]interface{}{
			"local-init": map[string]interface{}{
...
// Used to generate a docker compose file corresponding to the test environment
func (env *Config) genNodeService(compose testbed, name, image, dockerfilePath, envFile string, ports []string) {
	...
	compose.Services[nginxService] = map[string]interface{}{
		...
		"ports": ports,
		"volumes": []string{
			env.rootPath + "/node/cmd/resources/rate-limit-nginx.conf:/etc/nginx/templates/default.conf.template:ro",
		},
		"networks": []string{
			"eigenda-demo",
		},
...
func (env *Config) GenerateAllVariables() {
	...
	graphUrl := "http://graph-node:8000/subgraphs/name/Layr-Labs/eigenda-operator-state"

	// Create envs directory
	createDirectory(env.Path + "/envs")
	changeDirectory(env.rootPath + "/inabox-docker/")
	...
	// Generate local-init
	compose.Services["local-init"] = map[string]interface{}{
		"volumes": []string{
			env.Path + ":/data",
			env.rootPath + "/inabox-docker/secrets:/secrets",
			env.rootPath + "/inabox-docker/resources:/resources",
			env.rootPath + "/contracts:/contracts",
			env.rootPath + "/subgraphs:/subgraphs",
		},
		"build": Build{
			Context:    dockerBuildContext,
			Dockerfile: localinitDockerfile,
		},
		"extra_hosts": []string{
			"host.docker.internal:host-gateway",
		},
	...
	for i := 0; i < env.Services.Counts.NumDis; i++ {
		...
		env.genService(
			compose, name, disImage, disDockerfile,
	...
	for i := 0; i < env.Services.Counts.NumOpr; i++ {
		...
		env.genNodeService(
			compose, name, nodeImage, nodeDockerfile,
	...
	env.genService(
		compose, name, batcherImage, batcherDockerfile,
		filename, []string{})
	...
	env.genService(
		compose, name, encoderImage, encoderDockerfile,
	...
	name = "retriever"
	logPath, _, filename, envFile = env.getPaths(name)
	retrieverConfig := env.generateRetrieverVars(0, graphUrl, logPath, fmt.Sprint(port))
	writeEnv(retrieverConfig.getEnvMap(), envFile)
	env.Retriever = retrieverConfig

	_ = filename
	env.genService(
		compose, name, retrieverImage, retrieverDockerfile,
		filename, []string{fmt.Sprint(port)})
