...
type Config struct {
	...
	Churner    deploy.ChurnerVars
	Dispersers []deploy.DisperserVars
	Batcher    []deploy.BatcherVars
	Encoder    []deploy.EncoderVars
	Operators  []deploy.OperatorVars
	Stakers    []deploy.Staker
	Retriever  deploy.RetrieverVars
}
...
func NewTestConfig() (testEnv *Config) {
	testPath := "/data"
