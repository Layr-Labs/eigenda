...
import (
	...
	dynamodb_test_utils "github.com/Layr-Labs/eigenda/common/aws/dynamodb/utils"
	s3_test_utils "github.com/Layr-Labs/eigenda/common/aws/s3/utils"
...
func DeployResources(pool *dockertest.Pool, localStackPort, metadataTableName, bucketTableName string) error {
	...
	s3_test_utils.CheckOrCreateBucket(
		"localstack",
		"localstack",
	...
	_, err := dynamodb_test_utils.CreateTableIfNotExists(context.Background(), cfg, metadataTableName, blobstore.GenerateTableSchema(metadataTableName, 10, 10))
	if err != nil {
		return err
	}

	_, err = dynamodb_test_utils.CreateTableIfNotExists(context.Background(), cfg, bucketTableName, store.GenerateTableSchema(10, 10, bucketTableName))
	return err
}
