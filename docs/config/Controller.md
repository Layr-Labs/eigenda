...
| $${\color{red}\texttt{EnablePaymentAuthentication}}$$<br>`CONTROLLER_ENABLE_PAYMENT_AUTHENTICATION` | `bool`<br>`false` | If true, use the new payment authentication system running on the controller. If false, payment authentication is disabled and request validation will always fail |
...
| $${\color{red}\texttt{GrpcServerConfig.EnableServer}}$$<br>`CONTROLLER_GRPC_SERVER_CONFIG_ENABLE_SERVER` | `bool`<br>`false` | Enables/disables the gRPC server |
| $${\color{red}\texttt{GrpcServerConfig.GrpcPort}}$$<br>`CONTROLLER_GRPC_SERVER_CONFIG_GRPC_PORT` | `uint16`<br>`0` | Port that the gRPC server listens on |
| $${\color{red}\texttt{GrpcServerConfig.MaxGRPCMessageSize}}$$<br>`CONTROLLER_GRPC_SERVER_CONFIG_MAX_GRPC_MESSAGE_SIZE` | `int`<br>`0` | Maximum size of a gRPC message that the server will accept (in bytes) |
| $${\color{red}\texttt{GrpcServerConfig.MaxIdleConnectionAge}}$$<br>`CONTROLLER_GRPC_SERVER_CONFIG_MAX_IDLE_CONNECTION_AGE` | `time.Duration`<br>`0s` | Maximum time a connection can be idle before it is closed. |
| $${\color{red}\texttt{GrpcServerConfig.RequestMaxFutureAge}}$$<br>`CONTROLLER_GRPC_SERVER_CONFIG_REQUEST_MAX_FUTURE_AGE` | `time.Duration`<br>`0s` | Maximum age of a request in the future that the server will accept. Requests with timestamps too far in the future will be rejected. |
| $${\color{red}\texttt{GrpcServerConfig.RequestMaxPastAge}}$$<br>`CONTROLLER_GRPC_SERVER_CONFIG_REQUEST_MAX_PAST_AGE` | `time.Duration`<br>`0s` | Maximum age of a request in the past that the server will accept. Requests older than this will be rejected to prevent replay attacks. |
