	...
	if s.chunkAuthenticator != nil {
		...
		for _, blob := range batch.BlobCertificates {
			if payment_logic.IsOnDemandPayment(&blob.BlobHeader.PaymentMetadata) {
				...
				if in.GetDisperserID() != api.EigenLabsDisperserID {
					return nil, api.NewErrorForbidden(fmt.Sprintf("on-demand payments are only allowed for EigenLabsDisperser; receiving disperser ID: %d", in.GetDisperserID()))
				}
			}
		}
	}
