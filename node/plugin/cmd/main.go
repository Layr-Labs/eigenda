...
func pluginOps(ctx *cli.Context) {
	...
	if err != nil {
		log.Printf("Error: failed to get dispersal port from the socket: %s, err: %s", opSocket.String(), err)
		return
	}

	_, retrievalPort, err := net.SplitHostPort(opSocket.GetV1RetrievalSocket())
	if err != nil {
		log.Printf("Error: failed to get retrieval port from the socket: %s, err: %s", opSocket.String(), err)
		return
	}

	_, v2DispersalPort, err := net.SplitHostPort(opSocket.GetV2DispersalSocket())
	if err != nil {
		log.Printf("Error: failed to get v2 dispersal port from the socket: %s, err: %s", opSocket.String(), err)
		return
	}

	_, v2RetrievalPort, err := net.SplitHostPort(opSocket.GetV2RetrievalSocket())
	if err != nil {
		log.Printf("Error: failed to get v2 retrieval port from the socket: %s, err: %s", opSocket.String(), err)
		return
	}
