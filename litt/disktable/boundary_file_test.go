...
func TestBoundaryFile_LoadBoundaryFile_NonExistentFile(t *testing.T) {
	tempDir := t.TempDir()
	
	// Test loading lower bound file that doesn't exist
	lowerBoundary, err := LoadBoundaryFile(true, tempDir)
	...
	upperBoundary, err := LoadBoundaryFile(false, tempDir)
	require.NoError(t, err)
...
func TestBoundaryFile_LoadBoundaryFile_ExistingFile(t *testing.T) {
	tempDir := t.TempDir()
...
func TestBoundaryFile_LoadBoundaryFile_InvalidContent(t *testing.T) {
	tempDir := t.TempDir()
	...
	// Loading should fail
	_, err = LoadBoundaryFile(true, tempDir)
	require.Error(t, err)
	require.Contains(t, err.Error(), "failed to deserialize boundary file")
}

func TestBoundaryFile_Name(t *testing.T) {
	tempDir := t.TempDir()
...
func TestBoundaryFile_Path(t *testing.T) {
	tempDir := t.TempDir()
...
func TestBoundaryFile_Update(t *testing.T) {
	tempDir := t.TempDir()
	
	// Load boundary file (non-existent initially)
	boundary, err := LoadBoundaryFile(true, tempDir)
	require.NoError(t, err)
...
func TestBoundaryFile_Update_NilBoundary(t *testing.T) {
	var nilBoundary *BoundaryFile
	err := nilBoundary.Update(42)
	require.NoError(t, err) // Should not error on nil
}

func TestBoundaryFile_Write(t *testing.T) {
	tempDir := t.TempDir()
...
func TestBoundaryFile_Write_NilBoundary(t *testing.T) {
	var nilBoundary *BoundaryFile
	err := nilBoundary.Write()
	require.NoError(t, err) // Should not error on nil
}

func TestBoundaryFile_IsDefined(t *testing.T) {
	tempDir := t.TempDir()
...
func TestBoundaryFile_BoundaryIndex(t *testing.T) {
	tempDir := t.TempDir()
...
func TestBoundaryFile_Serialize(t *testing.T) {
	boundary := &BoundaryFile{
		lower:           false,
		parentDirectory: "/tmp",
		defined:         true,
...
func TestBoundaryFile_Deserialize(t *testing.T) {
	boundary := &BoundaryFile{
		lower:           true,
		parentDirectory: "/tmp",
		defined:         false,
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "failed to parse boundary index")
...
func TestBoundaryFile_RoundTrip(t *testing.T) {
	tempDir := t.TempDir()
	...
	boundary2, err := LoadBoundaryFile(true, tempDir)
	require.NoError(t, err)
	require.True(t, boundary2.IsDefined())
	require.Equal(t, uint32(98765), boundary2.BoundaryIndex())
}

func TestBoundaryFile_Constants(t *testing.T) {
	require.Equal(t, "lower-bound.txt", LowerBoundFileName)
	require.Equal(t, "upper-bound.txt", UpperBoundFileName)
}
