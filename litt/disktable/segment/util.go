...
// The boolean returned is true if the file already exists.
//
...
func verifyFilePermissions(path string) (bool, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			if err != nil {
				if os.IsNotExist(err) {
					return false, fmt.Errorf("parent directory %s does not exist", parentPath)
				}
				return false, fmt.Errorf("failed to stat parent directory %s: %w", parentPath, err)
			}

			if !parentInfo.IsDir() {
				return false, fmt.Errorf("parent directory %s is not a directory", parentPath)
			}

			if parentInfo.Mode()&0700 != 0700 {
				return false, fmt.Errorf("parent directory %s has insufficent permissions", parentPath)
			}
		}
	...
	if info.IsDir() {
		return false, fmt.Errorf("file %s is a directory", path)
	}
	if info.Mode()&0600 != 0600 {
		return false, fmt.Errorf("file %s has insufficent permissions", path)
	}
