...
func waitForSSH(t *testing.T, sshPort, privateKeyPath string) {
	...
	for i := 0; i < 30; i++ {
		session, err := NewSSHSession(logger, "testuser", "localhost", parsePort(sshPort), privateKeyPath, false)
		if err == nil {
			session.Close()
			return
		}
...
func TestSSHSession_NewSSHSession(t *testing.T) {
	...
	session, err := NewSSHSession(logger, "testuser", "localhost", parsePort(container.sshPort), container.privateKey, true)
	require.NoError(t, err)
	require.NotNil(t, session)
	defer session.Close()
	
	// Test with non-existent key
	_, err = NewSSHSession(logger, "testuser", "localhost", parsePort(container.sshPort), "/nonexistent/key", false)
	require.Error(t, err)
	require.Contains(t, err.Error(), "private key does not exist")
	
	// Test with wrong user
	_, err = NewSSHSession(logger, "wronguser", "localhost", parsePort(container.sshPort), container.privateKey, false)
	require.Error(t, err)
}
