...
//func writingBenchmark(b *testing.B, store kvstore.Store) {
//	keySize := 8
//	valueSize := 1024
...
//
//	_, err = os.Stat(kvstore.dbPath)
//	assert.True(b, os.IsNotExist(err))
//}

//func writeThenReadBenchmark(b *testing.B, store kvstore.KVStore) {
//	keySize := 8
//	valueSize := 1024
//
//	bytesToWrite := 1 * 1024 * 1024 * 1024 // 1 GB
...
//	assert.NoError(b, err)
//
//	store, err := kvstore.NewLevelStore(logger, kvstore.dbPath)
//	assert.NoError(b, err)
//
