...
func ValidateEncodingParams(params EncodingParams, blobLength, SRSOrder int) error {
	...
	// Check that the parameters are valid with respect to the size of the data input. The encoding parameters must equate to
	// a size that is at least as large as the size of the data input. Otherwise, the input data will not fit within the
	// encoded chunks.
