...
func TestMetererReservations(t *testing.T) {
	...
	paymentChainState.On("GetQuorumProtocolConfigs").Return(map[core.QuorumID]*core.PaymentQuorumProtocolConfig{
		0: {
			MinNumSymbols:              3,
	...
	for i, quorumNumber := range quoromNumbers {
		reservationWindow, err := mt.ChainPaymentState.GetReservationWindow(core.QuorumID(quorumNumber))
		assert.NoError(t, err)
		reservationPeriods[i] = meterer.GetReservationPeriodByNanosecond(now.UnixNano(), reservationWindow)
	}
	...
	reservationWindow, err := mt.ChainPaymentState.GetReservationWindow(meterer.OnDemandQuorumID)
	assert.NoError(t, err)
	...
	reservationWindow, err = mt.ChainPaymentState.GetReservationWindow(meterer.OnDemandQuorumID)
	assert.NoError(t, err)
	hugeDelay := time.Duration(4*reservationWindow) * time.Second
	header = createPaymentHeader(now.Add(-hugeDelay).UnixNano(), big.NewInt(0), accountID1)
	...
	for _, accountAndQuorum := range accountAndQuorums {
		...
		assert.NoError(t, err)
		reservationPeriod := meterer.GetReservationPeriodByNanosecond(now.UnixNano(), reservationWindow)
		overflowedReservationPeriod := meterer.GetOverflowPeriod(reservationPeriod, reservationWindow)
		item, err := dynamoClient.GetItem(ctx, reservationTableName, commondynamodb.Key{
			"AccountID":         &types.AttributeValueMemberS{Value: accountAndQuorum},
...
func TestMetererOnDemand(t *testing.T) {
	...
	minNumSymbols, err := mt.ChainPaymentState.GetMinNumSymbols(meterer.OnDemandQuorumID)
	assert.NoError(t, err)
	symbolsCharged := meterer.SymbolsCharged(symbolLength, minNumSymbols)
	pricePerSymbol, err := mt.ChainPaymentState.GetPricePerSymbol(meterer.OnDemandQuorumID)
	assert.NoError(t, err)
	...
	minNumSymbols, err = mt.ChainPaymentState.GetMinNumSymbols(meterer.OnDemandQuorumID)
	assert.NoError(t, err)
	symbolsCharged = meterer.SymbolsCharged(uint64(50), minNumSymbols)
	pricePerSymbol, err = mt.ChainPaymentState.GetPricePerSymbol(meterer.OnDemandQuorumID)
	assert.NoError(t, err)
	priceCharged = meterer.PaymentCharged(symbolsCharged, pricePerSymbol)
	header = createPaymentHeader(now.UnixNano(), priceCharged, accountID2)
	...
	minNumSymbols, err = mt.ChainPaymentState.GetMinNumSymbols(meterer.OnDemandQuorumID)
	assert.NoError(t, err)
	symbolsCharged = meterer.SymbolsCharged(uint64(30), minNumSymbols)
	pricePerSymbol, err = mt.ChainPaymentState.GetPricePerSymbol(meterer.OnDemandQuorumID)
	assert.NoError(t, err)
	priceCharged = meterer.PaymentCharged(symbolsCharged, pricePerSymbol)
	header = createPaymentHeader(now.UnixNano(), priceCharged, accountID2)
	...
	pricePerSymbol, err = mt.ChainPaymentState.GetPricePerSymbol(meterer.OnDemandQuorumID)
	assert.NoError(t, err)
	priceCharged = meterer.PaymentCharged(1010, pricePerSymbol)
	header = createPaymentHeader(now.UnixNano(), big.NewInt(0).Add(previousCumulativePayment, priceCharged), accountID1)
