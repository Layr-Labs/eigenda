...
func TestServerAccountLedgerReservations(t *testing.T) {
	...
	reservationPeriod := meterer.GetReservationPeriodByNanosecond(testTimestamp, reservationWindow)
	...
	currentReservationPeriod := meterer.GetReservationPeriodByNanosecond(now.UnixNano(), reservationWindow)
	overflowReservationPeriod := meterer.GetOverflowPeriod(currentReservationPeriod, reservationWindow)
	...
	pastReservationPeriod := meterer.GetReservationPeriodByNanosecond(testTimestampFromEarlierTest, reservationWindow)
	pastOverflowPeriod := meterer.GetOverflowPeriod(pastReservationPeriod, reservationWindow)
	...
	for i := 0; i < 9; i++ {
		...
		for _, accountAndQuorum := range accountAndQuorums {
			...
			expectedSymbolsCharged := meterer.SymbolsCharged(symbolLength, mockParams.QuorumProtocolConfigs[meterer.OnDemandQuorumID].MinNumSymbols)
	...
	overflowSymbolsCharged := meterer.SymbolsCharged(25, mockParams.QuorumProtocolConfigs[meterer.OnDemandQuorumID].MinNumSymbols)
	...
	overflowedReservationPeriod := meterer.GetOverflowPeriod(currentReservationPeriod, mockParams.QuorumProtocolConfigs[meterer.OnDemandQuorumID].ReservationRateLimitWindow)
...
func TestServerAccountLedgerRevertDebit(t *testing.T) {
	...
	symbolLength := uint64(100)
	pricePerSymbol := mockParams.QuorumPaymentConfigs[meterer.OnDemandQuorumID].OnDemandPricePerSymbol
	symbolsCharged := meterer.SymbolsCharged(symbolLength, mockParams.QuorumProtocolConfigs[meterer.OnDemandQuorumID].MinNumSymbols)
	priceCharged := meterer.PaymentCharged(symbolsCharged, pricePerSymbol)
