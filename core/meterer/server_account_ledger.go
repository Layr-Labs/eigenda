...
func (sal *ServerAccountLedger) loadPeriodRecords(ctx context.Context, params *PaymentVaultParams) error {
	...
	for quorumID := range sal.reservations {
		...
		if err != nil {
			...
			periods = append(periods, GetReservationPeriodByNanosecond(currentTime, 3600))
			continue
		}
		
		// Calculate current period using the proper reservation window
		currentPeriod := GetReservationPeriodByNanosecond(currentTime, protocolConfig.ReservationRateLimitWindow)
...
func (sal *ServerAccountLedger) processReservationRequest(
	...
	if err := ValidateReservations(sal.reservations, params.QuorumProtocolConfigs, quorumNumbers, header.Timestamp, receivedAt.UnixNano()); err != nil {
...
func (sal *ServerAccountLedger) RevertDebit(
	...
	for i, q := range quorumNumbers {
		...
		if err != nil {
			...
			// Use a default if we can't get the config
			quorumPeriods[quorumID] = GetReservationPeriodByNanosecond(header.Timestamp, 3600)
			quorumSizes[quorumID] = numSymbols // Fallback to raw symbols
		} else {
			// Calculate period using the proper reservation window
			quorumPeriods[quorumID] = GetReservationPeriodByNanosecond(header.Timestamp, protocolConfig.ReservationRateLimitWindow)
			// Calculate charged symbols exactly like meterer does
			quorumSizes[quorumID] = SymbolsCharged(numSymbols, protocolConfig.MinNumSymbols)
		}
	}
