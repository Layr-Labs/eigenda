...
func (sal *ServerLedger) ValidateGlobalOnDemandUsage(
	...
	period := GetReservationPeriodByNanosecond(timestampNs, 3600) // 1 hour window
	...
	for _, quorumConfig := range params.QuorumPaymentConfigs {
		globalLimit = GetBinLimit(quorumConfig.OnDemandSymbolsPerSecond, 3600) // 1 hour window
		break
	}
