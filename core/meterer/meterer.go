...
type Meterer struct {
	...
	// PaymentOffchainState tracks usage and payments in a storage backend
	PaymentOffchainState payment.PaymentOffchainState
...
func NewMeterer(
	...
	return &Meterer{
		...
		PaymentOffchainState: PaymentOffchainState,
...
func (m *Meterer) MeterRequest(ctx context.Context, header payment.PaymentMetadata, numSymbols uint64, quorumNumbers []uint8, receivedAt time.Time) (uint64, error) {
	...
	if !payment.IsOnDemandPayment(&header) {
	...
	_, protocolConfig, err := params.GetQuorumConfigs(payment.OnDemandDepositQuorumID)
	if err != nil {
...
func (m *Meterer) serveReservationRequest(
	ctx context.Context,
	globalParams *payment.PaymentVaultParams,
	header payment.PaymentMetadata,
	...
	if err := payment.ValidateReservations(reservations, globalParams.QuorumProtocolConfigs, quorumNumbers, header.Timestamp, receivedAt.UnixNano()); err != nil {
...
func (m *Meterer) incrementBinUsage(
	ctx context.Context, header payment.PaymentMetadata,
	reservations map[core.QuorumID]*payment.ReservedPayment,
	globalParams *payment.PaymentVaultParams,
	numSymbols uint64,
) error {
	...
	for quorumID := range reservations {
		...
		charges[quorumID] = payment.SymbolsCharged(numSymbols, protocolConfig.MinNumSymbols)
		quorumNumbers = append(quorumNumbers, quorumID)
		reservationWindows[quorumID] = protocolConfig.ReservationRateLimitWindow
		requestReservationPeriods[quorumID] = payment.GetReservationPeriodByNanosecond(header.Timestamp, protocolConfig.ReservationRateLimitWindow)
	}
	...
	usage, err := m.PaymentOffchainState.IncrementBinUsages(ctx, header.AccountID, quorumNumbers, requestReservationPeriods, charges)
	if err != nil {
		return err
	}
	...
	for quorumID, reservation := range reservations {
		...
		usageLimit := payment.GetBinLimit(reservation.SymbolsPerSecond, reservationWindow)
		...
		overflowPeriod := payment.GetOverflowPeriod(requestReservationPeriod, reservationWindow)
		if charges[quorumID] <= usageLimit && overflowPeriod <= payment.GetReservationPeriod(int64(reservation.EndTimestamp), reservationWindow) {
	...
	if len(overflowAmounts) > 0 {
		...
		_, err := m.PaymentOffchainState.IncrementBinUsages(ctx, header.AccountID, overflowQuorums, overflowPeriods, overflowAmounts)
		if err != nil {
			// Rollback the increments for the current periods
			rollbackErr := m.PaymentOffchainState.DecrementBinUsages(ctx, header.AccountID, quorumNumbers, requestReservationPeriods, charges)
			if rollbackErr != nil {
...
func (m *Meterer) serveOnDemandRequest(ctx context.Context, globalParams *payment.PaymentVaultParams, header payment.PaymentMetadata, onDemandPayment *payment.OnDemandPayment, symbolsCharged uint64, headerQuorums []uint8, receivedAt time.Time) error {
	m.logger.Debug("Recording and validating on-demand usage", "header", header, "onDemandPayment", onDemandPayment)

	if err := payment.ValidateQuorum(headerQuorums, globalParams.OnDemandQuorumNumbers); err != nil {
	...
	paymentConfig, protocolConfig, err := globalParams.GetQuorumConfigs(payment.OnDemandDepositQuorumID)
	if err != nil {
		return fmt.Errorf("failed to get payment config for on-demand quorum: %w", err)
	}

	symbolsCharged = payment.SymbolsCharged(symbolsCharged, protocolConfig.MinNumSymbols)
	paymentCharged := payment.PaymentCharged(symbolsCharged, paymentConfig.OnDemandPricePerSymbol)
	oldPayment, err := m.PaymentOffchainState.AddOnDemandPayment(ctx, header, paymentCharged)
	if err != nil {
	...
	if err := m.incrementGlobalBinUsage(ctx, globalParams, uint64(symbolsCharged), receivedAt); err != nil {
		...
		dbErr := m.PaymentOffchainState.RollbackOnDemandPayment(ctx, header.AccountID, header.CumulativePayment, oldPayment)
		if dbErr != nil {
			return dbErr
		}
...
func (m *Meterer) incrementGlobalBinUsage(ctx context.Context, params *payment.PaymentVaultParams, symbolsCharged uint64, receivedAt time.Time) error {
	paymentConfig, protocolConfig, err := params.GetQuorumConfigs(payment.OnDemandDepositQuorumID)
	if err != nil {
		return fmt.Errorf("failed to get quorum configs for on-demand quorum: %w", err)
	}

	globalPeriod := payment.GetReservationPeriod(receivedAt.Unix(), protocolConfig.OnDemandRateLimitWindow)

	newUsage, err := m.PaymentOffchainState.UpdateGlobalBin(ctx, globalPeriod, symbolsCharged)
	if err != nil {
		return fmt.Errorf("failed to increment global bin usage: %w", err)
	}
	if newUsage > payment.GetBinLimit(paymentConfig.OnDemandSymbolsPerSecond, protocolConfig.OnDemandRateLimitWindow) {
