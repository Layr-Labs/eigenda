...
func (m *Meterer) ServeReservationRequest(ctx context.Context, header core.PaymentMetadata, reservations map[uint8]*core.ReservedPayment, symbolsCharged uint64, quorumNumbers []uint8, receivedAt time.Time) error {
	...
	quorumIds := make([]uint8, 0, len(reservations))
	...
	for quorumId, reservation := range reservations {
		...
		if err := m.IncrementBinUsage(ctx, header, reservation, symbolsCharged, reservationWindow, requestReservationPeriod, quorumId); err != nil {
...
func (m *Meterer) IncrementBinUsage(ctx context.Context, header core.PaymentMetadata, reservation *core.ReservedPayment, symbolsCharged uint64, reservationWindow uint64, requestReservationPeriod uint64, quorumNumber uint8) error {
	newUsage, err := m.OffchainStore.UpdateReservationBin(ctx, header.AccountID, requestReservationPeriod, symbolsCharged, quorumNumber)
	if err != nil {
	...
	if newUsage <= 2*usageLimit && requestReservationPeriod+2 <= GetReservationPeriod(int64(reservation.EndTimestamp), reservationWindow) {
		_, err := m.OffchainStore.UpdateReservationBin(ctx, header.AccountID, uint64(requestReservationPeriod+2), newUsage-usageLimit, quorumNumber)
		if err != nil {
			return err
		}
		return nil
...
func (m *Meterer) IncrementGlobalBinUsage(ctx context.Context, symbolsCharged uint64, receivedAt time.Time) error {
	globalPeriod := GetReservationPeriod(receivedAt.Unix(), m.ChainPaymentState.GetOnDemandRatePeriodInterval())
	...
	if newUsage > m.ChainPaymentState.GetOnDemandSymbolsPerSecond()*uint64(m.ChainPaymentState.GetOnDemandRatePeriodInterval()) {
