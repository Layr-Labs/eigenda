...
func (m *Meterer) MeterRequest(ctx context.Context, header core.PaymentMetadata, numSymbols uint64, quorumNumbers []uint8, receivedAt time.Time) (uint64, error) {
	...
	globalParams, err := m.ChainPaymentState.GetPaymentGlobalParams()
	if err != nil {
	...
	if header.CumulativePayment.Sign() == 0 {
		...
		if err := m.serveReservationRequest(ctx, header, reservations, globalParams, numSymbols, quorumNumbers, receivedAt); err != nil {
		...
		if err := m.serveOnDemandRequest(ctx, header, onDemandPayment, globalParams, numSymbols, quorumNumbers, receivedAt); err != nil {
	...
	symbolsCharged := SymbolsCharged(numSymbols, globalParams.QuorumProtocolConfigs[OnDemandQuorumID].MinNumSymbols)
	return symbolsCharged, nil
}

// serveReservationRequest handles the rate limiting logic for incoming requests
func (m *Meterer) serveReservationRequest(ctx context.Context, header core.PaymentMetadata, reservations map[core.QuorumID]*core.ReservedPayment, globalParams *PaymentVaultParams, numSymbols uint64, quorumNumbers []uint8, receivedAt time.Time) error {
...
func (m *Meterer) incrementBinUsage(ctx context.Context, header core.PaymentMetadata, reservations map[core.QuorumID]*core.ReservedPayment, globalParams *PaymentVaultParams, numSymbols uint64) error {
	...
	for quorumID := range reservations {
		charges[quorumID] = SymbolsCharged(numSymbols, globalParams.QuorumProtocolConfigs[quorumID].MinNumSymbols)
		quorumNumbers = append(quorumNumbers, quorumID)
		reservationWindows[quorumID] = globalParams.QuorumProtocolConfigs[quorumID].ReservationRateLimitWindow
		requestReservationPeriods[quorumID] = GetReservationPeriodByNanosecond(header.Timestamp, globalParams.QuorumProtocolConfigs[quorumID].ReservationRateLimitWindow)
	}
...
func (m *Meterer) serveOnDemandRequest(ctx context.Context, header core.PaymentMetadata, onDemandPayment *core.OnDemandPayment, globalParams *PaymentVaultParams, symbolsCharged uint64, headerQuorums []uint8, receivedAt time.Time) error {
	...
	symbolsCharged = SymbolsCharged(symbolsCharged, globalParams.QuorumProtocolConfigs[OnDemandQuorumID].MinNumSymbols)
	paymentCharged := PaymentCharged(symbolsCharged, globalParams.QuorumPaymentConfigs[OnDemandQuorumID].OnDemandPricePerSymbol)
	...
	if err := m.incrementGlobalBinUsage(ctx, uint64(symbolsCharged), receivedAt, globalParams); err != nil {
...
func (m *Meterer) incrementGlobalBinUsage(ctx context.Context, symbolsCharged uint64, receivedAt time.Time, globalParams *PaymentVaultParams) error {
	globalPeriodInterval := globalParams.QuorumProtocolConfigs[OnDemandQuorumID].OnDemandRateLimitWindow
	...
	globalSymbolsPerSecond := globalParams.QuorumPaymentConfigs[OnDemandQuorumID].OnDemandSymbolsPerSecond
