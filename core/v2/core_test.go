...
func prepareBlobs(t *testing.T, operatorCount uint, headers []corev2.BlobCertificate, blobs [][]byte) (map[corev2.OperatorID][]*corev2.BlobShard, chainio.IndexedChainState) {
	...
	blobsMap := make([]map[corev2.QuorumID]map[corev2.OperatorID][]*encoding.Frame, 0, len(headers))
	...
	for z, header := range headers {
		...
		blobMap := make(map[corev2.QuorumID]map[corev2.OperatorID][]*encoding.Frame)
		...
		for _, quorum := range header.QuorumNumbers {
			...
			blobMap[quorum] = make(map[corev2.OperatorID][]*encoding.Frame)
	...
	// Invert the blobsMap
	inverseMap := make(map[corev2.OperatorID][]*corev2.BlobShard)
...
func TestValidationSucceeds(t *testing.T) {
	...
	quorumNumbers := []corev2.QuorumID{0, 1}
