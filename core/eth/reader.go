...
func NewReader(
	...
	usageAuthorityHexAddr string) (*Reader, error) {
	...
	var usageAuthorityAddr *gethcommon.Address
	if usageAuthorityHexAddr != "" {
		addr := gethcommon.HexToAddress(usageAuthorityHexAddr)
		usageAuthorityAddr = &addr
	}
	err := e.updateContractBindings(blsOperatorStateRetrieverAddr, eigenDAServiceManagerAddr, usageAuthorityAddr)
...
//
// Parameters:
//   - blsOperatorStateRetrieverAddr: the address of the BLS operator state retriever contract
//   - eigenDAServiceManagerAddr: the address of the EigenDA service manager contract
//   - usageAuthorityAddr: the address of the usage authority contract
//
// Returns:
//   - error: if the contract bindings fail to update
//
// TODO(hopeyen): remove usageAuthorityAddr from the function signature when the address registry is deployed
func (t *Reader) updateContractBindings(blsOperatorStateRetrieverAddr, eigenDAServiceManagerAddr gethcommon.Address, usageAuthorityAddr *gethcommon.Address) error {
	...
	var contractUsageAuthorizationRegistry *usageauthreg.ContractIUsageAuthorizationRegistry
	if usageAuthorityAddr == nil {
		t.logger.Error("Failed to fetch UsageAuthorizationRegistry address", "err", err)
		//TODO(hopeyen): return err when the contract is deployed
		// return err
	} else {
		contractUsageAuthorizationRegistry, err = usageauthreg.NewContractIUsageAuthorizationRegistry(*usageAuthorityAddr, t.ethClient)
		if err != nil {
			t.logger.Error("Failed to fetch UsageAuthorizationRegistry contract", "err", err)
			return err
		}
	}
...
func (t *Reader) GetReservedPaymentByAccountAndQuorums(ctx context.Context, accountID gethcommon.Address, quorumNumbers []core.QuorumID) (map[core.QuorumID]*core.ReservedPayment, error) {
	if t.bindings.UsageAuthorizationRegistry == nil {
		return nil, errors.New("usage authorization registry not deployed")
	}

	reservedPayments := make(map[core.QuorumID]*core.ReservedPayment)
	for _, quorumNumber := range quorumNumbers {
		// TODO: a plural version of this function should take in a list of quorumIds
		reservation, err := t.bindings.UsageAuthorizationRegistry.GetReservation(&bind.CallOpts{
			Context: ctx,
		}, uint64(quorumNumber), accountID)

		if err != nil {
			return nil, err
		}

		reservedPayment, err := ConvertToReservedPayments(reservation)
		if err != nil {
			return nil, err
		}

		reservedPayments[core.QuorumID(quorumNumber)] = reservedPayment
	}
...
func (t *Reader) GetQuorumProtocolConfig(ctx context.Context, quorumID core.QuorumID) (*core.PaymentQuorumProtocolConfig, error) {
	if t.bindings.PaymentVault == nil {
		return nil, errors.New("payment vault not deployed")
	}
	quorumProtocolConfig, err := t.bindings.UsageAuthorizationRegistry.GetQuorumProtocolConfig(&bind.CallOpts{
		Context: ctx,
	}, uint64(quorumID))
	if err != nil {
		return nil, err
	}
	return &core.PaymentQuorumProtocolConfig{
		MinNumSymbols:              quorumProtocolConfig.MinNumSymbols,
		ReservationAdvanceWindow:   quorumProtocolConfig.ReservationAdvanceWindow,
		ReservationRateLimitWindow: quorumProtocolConfig.ReservationRateLimitWindow,
		OnDemandRateLimitWindow:    quorumProtocolConfig.OnDemandRateLimitWindow,
		OnDemandEnabled:            quorumProtocolConfig.OnDemandEnabled,
	}, nil
}

func (t *Reader) GetQuorumPaymentConfig(ctx context.Context, quorumID core.QuorumID) (*core.PaymentQuorumConfig, error) {
	if t.bindings.PaymentVault == nil {
		return nil, errors.New("payment vault not deployed")
	}
	quorumPaymentConfig, err := t.bindings.UsageAuthorizationRegistry.GetQuorumPaymentConfig(&bind.CallOpts{
		Context: ctx,
	}, uint64(quorumID))
	if err != nil {
		return nil, err
	}
	return &core.PaymentQuorumConfig{
		ReservationSymbolsPerSecond: quorumPaymentConfig.ReservationSymbolsPerSecond,
		OnDemandSymbolsPerSecond:    quorumPaymentConfig.OnDemandSymbolsPerSecond,
		OnDemandPricePerSymbol:      quorumPaymentConfig.OnDemandPricePerSymbol,
	}, nil
}
