...
func TestTableList(t *testing.T) {
	...
	tStore, err := LevelDB.New(logger, dbPath)
	assert.NoError(t, err)
	...
	tStore, err = LevelDB.New(logger, dbPath, "table1")
	assert.NoError(t, err)
	...
	tStore, err = LevelDB.New(logger, dbPath, "table1", "table2")
	assert.NoError(t, err)
	tables = tStore.GetTables()
	...
	tStore, err = LevelDB.New(logger, dbPath, "table1", "table2", "table3")
	assert.NoError(t, err)
	tables = tStore.GetTables()
	...
	tStore, err = LevelDB.New(logger, dbPath, "table1", "table3")
	assert.NoError(t, err)
	tables = tStore.GetTables()
	assert.Equal(t, 2, len(tables))
	sort.SliceStable(tables, func(i, j int) bool {
	...
	tStore, err = LevelDB.New(logger, dbPath, "table1", "table3", "table4")
	assert.NoError(t, err)
	tables = tStore.GetTables()
	assert.Equal(t, 3, len(tables))
	sort.SliceStable(tables, func(i, j int) bool {
...
func TestUniqueKeySpace(t *testing.T) {
	...
	store, err := MapStore.New(logger, dbPath, "table1", "table2")
	assert.NoError(t, err)
...
func TestBatchOperations(t *testing.T) {
	...
	store, err := MapStore.New(logger, dbPath, "table1", "table2", "table3")
	assert.NoError(t, err)
...
func TestDropTable(t *testing.T) {
	...
	store, err := LevelDB.New(logger, dbPath, "table1", "table2", "table3")
	assert.NoError(t, err)
	...
	store, err = LevelDB.New(logger, dbPath, "table1", "table3")
	assert.NoError(t, err)
	...
	store, err = LevelDB.New(logger, dbPath, "table3")
	assert.NoError(t, err)
	...
	store, err = LevelDB.New(logger, dbPath)
	assert.NoError(t, err)
...
func TestRestart(t *testing.T) {
	...
	store, err := LevelDB.New(logger, dbPath, "table1", "table2")
	assert.NoError(t, err)
...
func TestRandomOperations(t *testing.T) {
	...
	for i := 0; i < 10000; i++ {
		...
		if choice < 0.01 {
			...
			store, err = LevelDB.New(logger, dbPath, getTableNameList(tables)...)
			assert.NoError(t, err)
			...
			store, err = LevelDB.New(logger, dbPath, tableNames...)
			assert.NoError(t, err)
