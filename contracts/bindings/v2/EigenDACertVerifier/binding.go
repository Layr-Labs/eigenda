...
// contract deployment.
//
// Solidity: constructor(address initEigenDAThresholdRegistry, address initEigenDASignatureVerifier, (uint8,uint8) initSecurityThresholds, bytes initQuorumNumbersRequired, uint8 initCertVersion, uint16 initOffchainDerivationVersion) returns()
func (contractEigenDACertVerifier *ContractEigenDACertVerifier) PackConstructor(initEigenDAThresholdRegistry common.Address, initEigenDASignatureVerifier common.Address, initSecurityThresholds EigenDATypesV1SecurityThresholds, initQuorumNumbersRequired []byte, initCertVersion uint8, initOffchainDerivationVersion uint16) []byte {
	enc, err := contractEigenDACertVerifier.abi.Pack("", initEigenDAThresholdRegistry, initEigenDASignatureVerifier, initSecurityThresholds, initQuorumNumbersRequired, initCertVersion, initOffchainDerivationVersion)
	if err != nil {
		panic(err)
	}
	return enc
...
// invalid/nil inputs are passed.
//
// Solidity: function certVersion() view returns(uint8)
...
type ContractEigenDACertVerifierInvalidOffchainDerivationVersion struct {
	ActualVersion   uint16
	ExpectedVersion uint16
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error InvalidOffchainDerivationVersion(uint16 actualVersion, uint16 expectedVersion)
