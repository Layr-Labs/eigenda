...
// contract deployment.
//
// Solidity: constructor(address initEigenDAThresholdRegistry, address initEigenDASignatureVerifier, (uint8,uint8) initSecurityThresholds, bytes initQuorumNumbersRequired, uint32 initRecencyWindow) returns()
func (contractEigenDACertVerifier *ContractEigenDACertVerifier) PackConstructor(initEigenDAThresholdRegistry common.Address, initEigenDASignatureVerifier common.Address, initSecurityThresholds EigenDATypesV1SecurityThresholds, initQuorumNumbersRequired []byte, initRecencyWindow uint32) []byte {
	enc, err := contractEigenDACertVerifier.abi.Pack("", initEigenDAThresholdRegistry, initEigenDASignatureVerifier, initSecurityThresholds, initQuorumNumbersRequired, initRecencyWindow)
	if err != nil {
		panic(err)
	}
	return enc
...
// PackRecencyWindow is the Go binding used to pack the parameters required for calling
// the contract method with ID 0xa0330cd6.  This method will panic if any
// invalid/nil inputs are passed.
//
// Solidity: function recencyWindow() view returns(uint32)
func (contractEigenDACertVerifier *ContractEigenDACertVerifier) PackRecencyWindow() []byte {
	enc, err := contractEigenDACertVerifier.abi.Pack("recencyWindow")
	if err != nil {
		panic(err)
	}
	return enc
...
// TryPackRecencyWindow is the Go binding used to pack the parameters required for calling
// the contract method with ID 0xa0330cd6.  This method will return an error
// if any inputs are invalid/nil.
//
// Solidity: function recencyWindow() view returns(uint32)
func (contractEigenDACertVerifier *ContractEigenDACertVerifier) TryPackRecencyWindow() ([]byte, error) {
	return contractEigenDACertVerifier.abi.Pack("recencyWindow")
}

// UnpackRecencyWindow is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0xa0330cd6.
//
// Solidity: function recencyWindow() view returns(uint32)
func (contractEigenDACertVerifier *ContractEigenDACertVerifier) UnpackRecencyWindow(data []byte) (uint32, error) {
	out, err := contractEigenDACertVerifier.abi.Unpack("recencyWindow", data)
	if err != nil {
