...
// contract deployment.
//
// Solidity: constructor(address initEigenDAThresholdRegistry, address initEigenDASignatureVerifier, (uint8,uint8) initSecurityThresholds, bytes initQuorumNumbersRequired, uint32 initCheckDACertGasLimit) returns()
func (contractEigenDACertVerifier *ContractEigenDACertVerifier) PackConstructor(initEigenDAThresholdRegistry common.Address, initEigenDASignatureVerifier common.Address, initSecurityThresholds EigenDATypesV1SecurityThresholds, initQuorumNumbersRequired []byte, initCheckDACertGasLimit uint32) []byte {
	enc, err := contractEigenDACertVerifier.abi.Pack("", initEigenDAThresholdRegistry, initEigenDASignatureVerifier, initSecurityThresholds, initQuorumNumbersRequired, initCheckDACertGasLimit)
	if err != nil {
		panic(err)
	}
	return enc
...
// PackCheckDACertGasLimit is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x8237f542.  This method will panic if any
// invalid/nil inputs are passed.
//
// Solidity: function checkDACertGasLimit() view returns(uint32)
func (contractEigenDACertVerifier *ContractEigenDACertVerifier) PackCheckDACertGasLimit() []byte {
	enc, err := contractEigenDACertVerifier.abi.Pack("checkDACertGasLimit")
	if err != nil {
		panic(err)
	}
	return enc
...
// TryPackCheckDACertGasLimit is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x8237f542.  This method will return an error
// if any inputs are invalid/nil.
//
// Solidity: function checkDACertGasLimit() view returns(uint32)
func (contractEigenDACertVerifier *ContractEigenDACertVerifier) TryPackCheckDACertGasLimit() ([]byte, error) {
	return contractEigenDACertVerifier.abi.Pack("checkDACertGasLimit")
}

// UnpackCheckDACertGasLimit is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x8237f542.
//
// Solidity: function checkDACertGasLimit() view returns(uint32)
func (contractEigenDACertVerifier *ContractEigenDACertVerifier) UnpackCheckDACertGasLimit(data []byte) (uint32, error) {
	out, err := contractEigenDACertVerifier.abi.Unpack("checkDACertGasLimit", data)
	if err != nil {
