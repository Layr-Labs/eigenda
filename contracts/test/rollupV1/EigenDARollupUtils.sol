...
library EigenDARollupUtils {
    ...
    function verifyBlobs(
        ...
        require(
            ...
            "EigenDARollupUtils.verifyBlobs: blobHeaders and blobVerificationProofs must have the same length"
        );

        bytes memory quorumAdversaryThresholdPercentages = eigenDAServiceManager.quorumAdversaryThresholdPercentages();
        uint256 quorumNumbersRequiredBitmap =
            BitmapUtils.orderedBytesArrayToBitmap(eigenDAServiceManager.quorumNumbersRequired());
        ...
        for (uint256 i = 0; i < blobHeaders.length; i++) {
            ...
            for (uint256 j = 0; j < blobHeaders[i].quorumBlobParams.length; j++) {
                ...
                require(
                    uint8(
                        blobVerificationProofs[i].batchMetadata.batchHeader.quorumNumbers[uint8(
                            blobVerificationProofs[i].quorumIndices[i]
                        )]
                    ) == blobHeaders[i].quorumBlobParams[i].quorumNumber,
                    "EigenDARollupUtils.verifyBlob: quorumNumber does not match"
                );

                // make sure that the adversaryThresholdPercentage is less than the given confirmationThresholdPercentage
                require(
                    blobHeaders[i].quorumBlobParams[i].adversaryThresholdPercentage
                ...
                // make sure that the adversaryThresholdPercentage is at least the given quorumAdversaryThresholdPercentage
                uint8 _adversaryThresholdPercentage =
                    uint8(quorumAdversaryThresholdPercentages[blobHeaders[i].quorumBlobParams[j].quorumNumber]);
                if (_adversaryThresholdPercentage > 0) {
                    require(
                        blobHeaders[i].quorumBlobParams[j].adversaryThresholdPercentage >= _adversaryThresholdPercentage,
                        "EigenDARollupUtils.verifyBlob: adversaryThresholdPercentage is not met"
                    );
                }
            ...
            // check that required quorums are a subset of the confirmed quorums
            require(
                BitmapUtils.isSubsetOf(quorumNumbersRequiredBitmap, confirmedQuorumsBitmap),
                "EigenDARollupUtils.verifyBlob: required quorums are not a subset of the confirmed quorums"
            );
        }
    }
