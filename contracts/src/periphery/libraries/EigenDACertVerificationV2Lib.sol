...
library EigenDACertVerificationV2Lib {
     ...
     */
    function verifyBlobInclusion(BatchHeaderV2 memory batchHeader, BlobInclusionInfo memory blobInclusionInfo)
        internal
        pure
        returns (ErrorCode err, bytes memory errParams)
    {
        ...
        if (isValid) {
            ...
            return
                (ErrorCode.INVALID_INCLUSION_PROOF, abi.encode(blobInclusionInfo.blobIndex, encodedBlobHash, rootHash));
        }
    }
     ...
     * @return errParams Additional error parameters
     */
    function verifySecurityParams(VersionedBlobParams memory blobParams, SecurityThresholds memory securityThresholds)
        internal
        pure
        returns (ErrorCode err, bytes memory errParams)
    {
        ...
        if (n >= minRequired) {
            ...
            return (ErrorCode.SECURITY_ASSUMPTIONS_NOT_MET, abi.encode(gamma, n, minRequired));
        }
    }

    /**
     * @notice Verifies that all relay keys are set
     * @param relayRegistry The relay registry contract
     * @param relayKeys Array of relay keys to verify
     * @return err Error code (SUCCESS if verification succeeded)
     * @return errParams Additional error parameters
     */
    function verifyRelayKeysSet(IEigenDARelayRegistry relayRegistry, uint32[] memory relayKeys)
        internal
        view
        returns (ErrorCode err, bytes memory errParams)
    {
        for (uint256 i = 0; i < relayKeys.length; ++i) {
            if (relayRegistry.relayKeyToAddress(relayKeys[i]) == address(0)) {
                return (ErrorCode.RELAY_KEY_NOT_SET, abi.encode(relayKeys[i]));
            }
        }
        return (ErrorCode.SUCCESS, "");
    }
    ...
    /**
     * @notice Verifies quorum signatures and builds a bitmap of confirmed quorums
     ...
     */
    function verifySignaturesAndBuildConfirmedQuorums(
    ...
    ) internal view returns (ErrorCode err, bytes memory errParams, uint256 confirmedQuorumsBitmap) {
    ...
    /**
     * @notice Verifies that blob quorums are a subset of confirmed quorums
     ...
     */
    function verifyBlobQuorumsSubset(bytes memory blobQuorumNumbers, uint256 confirmedQuorumsBitmap)
        internal
        pure
        returns (ErrorCode err, bytes memory errParams, uint256 blobQuorumsBitmap)
    {
        ...
        if (BitmapUtils.isSubsetOf(blobQuorumsBitmap, confirmedQuorumsBitmap)) {
            return (ErrorCode.SUCCESS, "", blobQuorumsBitmap);
        } else {
            return (ErrorCode.BLOB_QUORUMS_NOT_SUBSET, abi.encode(blobQuorumsBitmap, confirmedQuorumsBitmap), 0);
        }
    }

    /**
     * @notice Verifies that required quorums are a subset of blob quorums
     * @param requiredQuorumNumbers The required quorum numbers
     * @param blobQuorumsBitmap The bitmap of blob quorums
     * @return err Error code (SUCCESS if verification succeeded)
     * @return errParams Additional error parameters
     */
    function verifyRequiredQuorumsSubset(bytes memory requiredQuorumNumbers, uint256 blobQuorumsBitmap)
        internal
        pure
        returns (ErrorCode err, bytes memory errParams)
    {
        ...
        if (BitmapUtils.isSubsetOf(requiredQuorumsBitmap, blobQuorumsBitmap)) {
            ...
            return (ErrorCode.REQUIRED_QUORUMS_NOT_SUBSET, abi.encode(requiredQuorumsBitmap, blobQuorumsBitmap));
        }
    }

    /**
     * @notice Verifies a complete blob certificate for V2 in a single call
    ...
    function verifyDACertV2(
        ...
        bytes memory signedQuorumNumbers
    ) internal view returns (ErrorCode err, bytes memory errParams) {
        // Verify blob inclusion
        (err, errParams) = verifyBlobInclusion(batchHeader, blobInclusionInfo);
        if (err != ErrorCode.SUCCESS) {
            return (err, errParams);
        }

        // Verify relay keys are set
        (err, errParams) = verifyRelayKeysSet(relayRegistry, blobInclusionInfo.blobCertificate.relayKeys);
        if (err != ErrorCode.SUCCESS) {
            return (err, errParams);
        }

        // Verify security params
        (err, errParams) = verifySecurityParams(
        ...
        (err, errParams, confirmedQuorumsBitmap) = verifySignaturesAndBuildConfirmedQuorums(
            signatureVerifier,
        ...
        (err, errParams, blobQuorumsBitmap) =
            verifyBlobQuorumsSubset(blobInclusionInfo.blobCertificate.blobHeader.quorumNumbers, confirmedQuorumsBitmap);
        if (err != ErrorCode.SUCCESS) {
            return (err, errParams);
        }

        // Verify required quorums are a subset of blob quorums
        return verifyRequiredQuorumsSubset(requiredQuorumNumbers, blobQuorumsBitmap);
    }
    ...
    /**
     * @notice Verifies a blob certificate using a signed batch
     * @param eigenDAThresholdRegistry The threshold registry contract
     * @param signatureVerifier The signature verifier contract
     * @param relayRegistry The relay registry contract
     * @param operatorStateRetriever The operator state retriever contract
    ...
    function revertOnError(ErrorCode err, bytes memory errParams) internal pure {
        ...
        if (err == ErrorCode.INVALID_INCLUSION_PROOF) {
