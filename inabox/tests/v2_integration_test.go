	...
	It("test end to end scenario", func() {
		...
		for loop := true; loop; {
			...
			case <-ticker.C:
				...
				Expect(err).To(BeNil())
				blobKey, err := blobCert1.BlobHeader.BlobKey()
				Expect(err).To(BeNil())
				fmt.Println("====================== certhash ", blobVerification.GetBlobIndex(), hex.EncodeToString(certHash[:]), batchHeader.BatchRoot, blobKey.Hex())
				fmt.Println("==================== proof", inclusionProofBytes)
				fmt.Println("==================== blob header commitment", blobCert1.BlobHeader.BlobCommitments.Commitment.X, blobCert1.BlobHeader.BlobCommitments.Commitment.Y)
				fmt.Println("==================== payment metadata", blobCert1.BlobHeader.PaymentMetadata.AccountID, blobCert1.BlobHeader.PaymentMetadata.BinIndex, blobCert1.BlobHeader.PaymentMetadata.CumulativePayment.String())
				fmt.Println("==================== signature", blobCert1.BlobHeader.Signature)
				fmt.Println("==================== quorum numbers", blobCert1.BlobHeader.QuorumNumbers)
				...
				Expect(err).To(BeNil())
				fmt.Println("====================== certhash 2", hex.EncodeToString(certHash[:]))
				verified, err = merkletree.VerifyProofUsing(certHash[:], false, proof, [][]byte{batchHeader.BatchRoot}, keccak256.New())
				Expect(err).To(BeNil())
				Expect(verified).To(BeTrue())
		...
		for relayKey := range relays {
			fmt.Println("===== requesting blob from relay", relayKey)
			blob1, err := relayClient.GetBlob(ctx, relayKey, key1)
			fmt.Println("====== err from relay", err)
