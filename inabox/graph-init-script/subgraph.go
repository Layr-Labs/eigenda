...
func updateSubgraph(updater subgraphUpdater, path string, startBlock int) {
	...
	networkData := utils.MustReadFile(networkFile)
	...
	utils.MustWriteFile(networkFile, networkJson)
	log.Print("networks.json written")

	// Update subgraph.yaml
	subgraphFile := filepath.Join(path, "subgraph.yaml")
	subgraphTemplateData := utils.MustReadFile(subgraphFile)
	...
	utils.MustWriteFile(subgraphFile, subgraphYaml)
