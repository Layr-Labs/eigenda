...
func CreateNewTestDirectory(templateName, rootPath string) (string, error) {
	...
	testPath := filepath.Join(rootPath, fmt.Sprintf("inabox/strategies/processes/testdata/%s", testName))
	...
	templatePath := filepath.Join(rootPath, fmt.Sprintf("inabox/strategies/processes/templates/%s", templateName))
	err = execCmd(
		"cp",
...
func GetLatestTestDirectory(rootPath string) (string, error) {
	files, err := os.ReadDir(filepath.Join(rootPath, "inabox/strategies/processes/testdata"))
	if err != nil {
		return "", err
	}
	if len(files) == 0 {
